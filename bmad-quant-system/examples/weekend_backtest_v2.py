"""
Weekend Pricing Strategy Full Backtest V2
==========================================
...
PnL calculation:
- For JPY: PnL(CNH) = spread_earned(bps) * 0.0001 * JPYCNH_rate * JPY_volume
  每笔交易的损益 = 点差收益(bps) * JPYCNH报价 * 交易量
...
print("=" * 80)
print("Weekend Pricing Strategy - Full Backtest V2 (1 Year)")
print("=" * 80)
...
def load_existing_data():
    ...
    # Find the most recent JPYCNH and USDCNH files (prefer 1year files)
    all_files = os.listdir(OUTPUT_DIR)
    
    # Filter for JPYCNH files
...
def backtest_weekend_strategy(jpycnh_sat, usdcnh_sat, common_dates, spread_bps=5.0):
    ...
    - 如果报价误差在半点差内，赚取半点差
    - 否则，损失为 (报价误差 - 半点差)
    - 最终损益 = PnL(bps) * 0.0001 * JPYCNH价格 * JPY交易量
    """
    ...
    for adj_factor in adjustment_factors:
        ...
        for sat_date in common_dates:
            ...
            # Calculate PnL in CNH
            # JPYCNH报价约为0.05 (100日元换约5元人民币，所以1日元约0.05元)
            jpycnh_rate = jpycnh_day['mid'].mean()  # 约 0.045-0.050
            usdcnh_rate = usdcnh_day['mid'].mean()  # 约 7.2-7.3
            
            # PnL in CNH = PnL(bps) * 0.0001 * JPY_volume * JPYCNH_rate
            # 但是这里PnL是每分钟的累计，我们需要把它当作整周的报价损益
            # 实际上：每周交易4B JPY，报价收益/损失按照平均报价误差计算
            
            # 简化计算：周损益(CNH) = 平均每笔PnL(bps) * 0.0001 * 周交易量(JPY) * JPYCNH价格
            avg_pnl_per_trade_bps = week_pnl_bps / week_trades if week_trades > 0 else 0
            week_pnl_cnh = avg_pnl_per_trade_bps * 0.0001 * JPY_WEEKLY_VOLUME * jpycnh_rate
...
def plot_backtest_results(all_results, output_dir):
    ...
    # 3. Average Weekly PnL (CNH) vs Spread - Best factor only
    ax3 = axes[1, 0]
    best_factors = []
    best_pnls = []
    spreads_list = []
    ...
    ax3.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x/1e3:.0f}K' if abs(x) >= 1e3 else f'{x:.0f}'))
    ...
    for spread, df in all_results.items():
        ...
        summary_text += f"    Total: {best['total_pnl_cnh']/1e6:.2f}M CNH\n"
        summary_text += f"    Weeks: {best['num_weeks']:.0f}\n"
    ...
    save_path = os.path.join(output_dir, f'weekend_backtest_v2_{timestamp}.png')
...
def save_results_to_excel(all_results, df_weekly, output_dir):
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_file = os.path.join(output_dir, f'weekend_backtest_v2_{timestamp}.xlsx')
...
def main():
    ...
    for spread, df in all_results.items():
        best_idx = df['total_pnl_cnh'].idxmax()
        best = df.loc[best_idx]
        print(f"{spread:>10.0f} {best['adj_factor']:>8.2f} {best['win_rate']*100:>7.1f}% {best['num_weeks']:>8.0f} {best['total_pnl_cnh']:>15,.0f} {best['avg_weekly_pnl_cnh']:>15,.0f}")
    ...
    for spread, df in all_results.items():
        ...
        print(f"  Spread {spread:.0f}bps: ~{annual_pnl/1e6:.2f} million CNH / year")
