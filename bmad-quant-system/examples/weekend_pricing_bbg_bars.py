...
# -*- coding: utf-8 -*-
"""
周末报价方案对比分析 - 使用 Bloomberg Bar 数据 (10秒间隔)
===============================================================

使用 IntradayBarRequest 获取 BID/ASK 的 Bar 数据，
支持更长时间范围的历史数据下载

数据: USDCNH Curncy 的 ASK Bar 数据 (10秒间隔)
时间: 过去一年的所有周五晚上到周六凌晨
...
def download_weekend_data_bars(bbg, symbol: str, friday: datetime, saturday: datetime) -> pd.DataFrame:
    """
    下载单个周末的 Bar 数据
    时间范围: 周五 22:00 到 周六 06:00 (北京时间)
    
    使用 get_bid_ask_bars 获取 10 秒间隔的 bar 数据
    """
    # 下载时间范围: 周五 22:00 到 周六 06:00
    start_dt = datetime.combine(friday, datetime.strptime("22:00", "%H:%M").time())
    end_dt = datetime.combine(saturday, datetime.strptime("06:30", "%H:%M").time())
    
    print(f"  Downloading: {start_dt.strftime('%Y-%m-%d %H:%M')} ~ {end_dt.strftime('%Y-%m-%d %H:%M')}")
    
    # 使用 get_bid_ask_bars 获取 10 秒间隔的数据
...
def analyze_weekend_pricing(data: pd.DataFrame, saturday_date) -> dict:
    ...
    # 原方案: 02:00-06:00
    original_window = saturday_data.between_time("02:00", "05:59:59")
    
    # 新方案: 00:00-06:00
    new_window = saturday_data.between_time("00:00", "05:59:59")
...
def run_analysis_with_bbg():
    ...
    print("=" * 60)
    print("周末报价方案对比分析 - Bloomberg Bar 数据 (10秒间隔)")
    print("=" * 60)
        ...
        try:
            ...
            if data is not None and not data.empty:
                ...
                if result:
                    ...
                    print(f"  Diff:     {result['quote_diff']*10000:.2f} pips")
            else:
    ...
    if not better_records.empty:
        ...
        for _, row in better_records.iterrows():
            print(f"  {row['date']}: 原方案={row['original_quote']:.4f}@{row['original_max_time']}, "
                  f"新方案={row['new_quote']:.4f}@{row['new_max_time']}, "
                  f"差异={row['quote_diff_pips']:.1f}pips, PnL=${row['pnl_diff']:,.0f}")
    ...
    output_file = output_dir / f"weekend_pricing_bbg_bars_{timestamp}.xlsx"
    ...
    if all_data:
        ...
        data_file = output_dir / f"weekend_raw_data_bbg_bars_{timestamp}.csv"
